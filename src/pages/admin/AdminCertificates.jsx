...
export default function AdminCertificates() {
  ...
  // State for Team Certificates Tab
  const [searchQuery, setSearchQuery] = useState('');
  const [teamStatusFilter, setTeamStatusFilter] = useState('all'); // 'all' | 'present' | 'absent'
  ...
  const absentTeams = teams.filter(t => !isTeamPresent(t));

  const filteredTeamsList = useMemo(() => {
    return teams.filter(t => {
      const isPresent = isTeamPresent(t);
      if (teamStatusFilter === 'present' && !isPresent) return false;
      if (teamStatusFilter === 'absent' && isPresent) return false;
  ...
  }, [teams, teamStatusFilter, searchQuery]);
  ...
  const handleBulkDownloadFilteredTeams = async () => {
    if (filteredTeamsList.length === 0) return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap bg-slate-900 p-1.5 rounded-2xl border border-slate-800 gap-1 text-sm font-semibold">
        ...
        >
          ...
          <span>Team Certificates ({teams.length})</span>
        </button>
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {/* ───────────────────────────────────────────────────────────── */}
      {/* TAB 2: TEAM CERTIFICATES & BUNDLE GENERATION                  */}
      {/* ───────────────────────────────────────────────────────────── */}
      {activeTab === 'present_teams' && (
        <div className="space-y-4">
          {/* Controls Bar */}
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-400">
              ...
              <div className="flex items-center space-x-1 bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg transition-colors ${teamStatusFilter === 'all' ? 'bg-slate-800 text-white font-bold' : 'text-slate-400 hover:text-white'}`}
                >
                  All ({teams.length})
                </button>
                <button
                  onClick={() => setTeamStatusFilter('present')}
                  className={`px-3 py-1.5 rounded-lg transition-colors ${teamStatusFilter === 'present' ? 'bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30' : 'text-slate-400 hover:text-white'}`}
                >
                  Present ({presentTeams.length})
                </button>
                <button
                  onClick={() => setTeamStatusFilter('absent')}
                  className={`px-3 py-1.5 rounded-lg transition-colors ${teamStatusFilter === 'absent' ? 'bg-rose-500/20 text-rose-300 font-bold border border-rose-500/30' : 'text-slate-400 hover:text-white'}`}
                >
                  Absent ({absentTeams.length})
                </button>
              </div>

              <button
                onClick={handleBulkDownloadFilteredTeams}
                disabled={bulkDownloading || filteredTeamsList.length === 0}
                className="flex items-center space-x-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs rounded-xl transition-all shadow-md cursor-pointer"
              >
                <Download className="w-3.5 h-3.5" />
                <span>{bulkDownloading ? 'Exporting...' : `Export Bundles (${filteredTeamsList.length})`}</span>
              </button>
              <button
                onClick={loadTeams}
                disabled={loadingTeams}
          ...
          {/* Teams Cards & Certificate Generator */}
          {loadingTeams ? (
            ...
            <div className="py-16 text-center text-slate-500 bg-slate-900 rounded-2xl border border-slate-800">
              <p>No teams found matching your query or filter.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredTeamsList.map((team, idx) => {
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center space-x-2">
                          <span className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-mono font-bold rounded">
                          ...
                          {isPresent ? (
                            ...
                            <span className="px-2 py-0.5 bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[10px] font-extrabold rounded-full uppercase">
                              ABSENT
                            </span>
                          )}
                        </div>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold rounded-xl transition-colors flex items-center space-x-1.5 cursor-pointer"
                        ...
                        {members.map((m, mIdx) => (
                          <div key={m.id || mIdx} className="flex items-center justify-between p-2 bg-slate-950 rounded-xl border border-slate-800/80">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-emerald-400 text-[11px] font-bold rounded-lg border border-slate-700 transition-colors flex items-center space-x-1 cursor-pointer"
                            >
